...
/* v34 - Encerrar turno robusto: listener direto, fallback inline e modal criado por DOM */
function abrirModalEncerrarTurno(){
  try{ document.querySelectorAll('.modal-backdrop').forEach(m=>m.remove()); }catch(e){}
  ...
  modal.innerHTML = `
    <h3>⚠️ Encerrar turno</h3>
    <p>Tem certeza que deseja encerrar o turno?</p>
    <p class="mini-note">Os chamados e as pendências continuam salvos neste aparelho. Apenas os dados de login do operador serão apagados.</p>
    <div class="actions">
      <button type="button" class="btn status-cancelado" id="btnConfirmarEncerrarTurnoFinal">Sim, encerrar turno</button>
      <button type="button" class="btn secondary" id="btnCancelarEncerrarTurnoFinal">Cancelar</button>
    </div>`;
  backdrop.appendChild(modal);
  document.body.appendChild(backdrop);
  const confirmar = document.getElementById('btnConfirmarEncerrarTurnoFinal');
  const cancelar = document.getElementById('btnCancelarEncerrarTurnoFinal');
  if(confirmar){ confirmar.onclick = function(ev){ if(ev){ev.preventDefault();ev.stopPropagation();} try{ document.querySelectorAll('.modal-backdrop').forEach(m=>m.remove()); }catch(e){} encerrarTurno(); return false; }; }
  if(cancelar){ cancelar.onclick = function(ev){ if(ev){ev.preventDefault();ev.stopPropagation();} try{ document.querySelectorAll('.modal-backdrop').forEach(m=>m.remove()); }catch(e){} return false; }; }
  backdrop.onclick = function(ev){ if(ev.target === backdrop){ try{ document.querySelectorAll('.modal-backdrop').forEach(m=>m.remove()); }catch(e){} } };
}
function confirmarEncerrarTurno(){ abrirModalEncerrarTurno(); return false; }
function bindEncerrarTurno(){
  const btn = document.getElementById('btnEncerrarTurno') || document.querySelector('[data-action="encerrar-turno"]');
  if(!btn) return;
  btn.onclick = function(ev){ if(ev){ev.preventDefault();ev.stopPropagation();} abrirModalEncerrarTurno(); return false; };
  btn.ontouchend = function(ev){ if(ev){ev.preventDefault();ev.stopPropagation();} abrirModalEncerrarTurno(); return false; };
}
document.addEventListener('DOMContentLoaded', bindEncerrarTurno);
window.addEventListener('load', bindEncerrarTurno);
setTimeout(bindEncerrarTurno, 100);
window.confirmarEncerrarTurno = confirmarEncerrarTurno;
window.abrirModalEncerrarTurno = abrirModalEncerrarTurno;
