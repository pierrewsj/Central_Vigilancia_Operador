...
let selectedId = '';
const OPERADOR_KEY='gsp_operador_logado_v19';
const CHAMADOS_CACHE_KEY='gsp_operador_chamados_cache_v19';
const STATUS_QUEUE_KEY='gsp_status_queue_v19';
...
async function processStatusQueue(){
  ...
  saveQueue(pending);
  if(q.length && !pending.length){ showModal('Sincronização concluída','As alterações pendentes foram enviadas para a planilha.','✅'); carregarChamados(); }
}
...
function bootOperador(){
  ...
  if(saved){
    ...
    if(operador && operador.nome){
      $('opNome').value=operador.nome||''; $('opRegistro').value=operador.registro||''; $('opTurno').value=operador.turno||'1º Turno';
...
window.addEventListener('online',()=>{setOfflineState();processStatusQueue();carregarChamados();});
...
async function carregarChamados(){
  ...
  if(!navigator.onLine){
    setOfflineState();
    try{ chamados=JSON.parse(localStorage.getItem(CHAMADOS_CACHE_KEY)||'[]'); }catch(e){ chamados=[]; }
    render();
    if(lista && !chamados.length) lista.innerHTML='<div class="card">Sem internet. Nenhum chamado em cache neste aparelho.</div>';
    return;
  }
  ...
  try{
    ...
    try{ chamados=JSON.parse(localStorage.getItem(CHAMADOS_CACHE_KEY)||'[]'); }catch(_){ chamados=[]; }
    render();
    const aviso=`<div class="card">Erro ao atualizar planilha: ${e.message||e}<br><span class="mini-note">Mostrando dados salvos neste aparelho, se existirem.</span></div>`;
...
function renderStats(){
  const data=[
    ['Recebidos',statusCount('Recebido'),'status-recebido'],
    ['Em desloc.',statusCount('Em deslocamento'),'status-deslocamento'],
    ['Em atend.',statusCount('Em atendimento'),'status-atendimento'],
    ['Aguard.',statusCount('Aguardando'),'status-aguardando'],
    ['Finaliz.',statusCount('Finalizado'),'status-finalizado'],
    ['Cancel.',statusCount('Cancelado'),'status-cancelado']
  ];
  $('stats').className='stats-status';
  $('stats').innerHTML=data.map(([label,total,cls])=>`<div class="card stat-mini ${cls}"><span>${label}</span><strong>${total}</strong></div>`).join('');
}
function render(){
  renderStats();
