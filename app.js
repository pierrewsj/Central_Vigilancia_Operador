...
function confirmarEncerrarTurno(){
  showCustomModal(`
    <h3>⚠️ Encerrar turno</h3>
    <p>Tem certeza que deseja encerrar o turno?</p>
    <p class="mini-note">Os chamados e as pendências serão mantidos neste aparelho. Apenas os dados de login do operador serão apagados.</p>
    <div class="actions"><button type="button" class="btn status-cancelado" id="confirmEncerrarTurnoBtn">Sim, encerrar turno</button><button type="button" class="btn secondary" onclick="closeCustomModal()">Cancelar</button></div>
  `, ()=>{
    const btn=$('confirmEncerrarTurnoBtn');
    if(btn){ btn.onclick=()=>{ closeCustomModal(); encerrarTurno(); }; }
  });
}
